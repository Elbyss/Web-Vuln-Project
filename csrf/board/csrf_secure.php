...
<?php
		...
		if($page != 1) { 
			$paging .= '<li class="page page_start"><a href="?page=1' . $subString . '">처음</a></li>';
		}
		//첫 섹션이 아니라면 이전 버튼을 생성
		if($currentSection != 1) { 
			$paging .= '<li class="page page_prev"><a href="?page=' . $prevPage . $subString . '">이전</a></li>';
		}
		...
		for($i = $firstPage; $i <= $lastPage; $i++) {
			if($i == $page) {
				...
				$paging .= '<li class="page"><a href="?page=' . $i . $subString . '">' . $i . '</a></li>';
			}
		}
		
		//마지막 섹션이 아니라면 다음 버튼을 생성
		if($currentSection != $allSection) { 
			$paging .= '<li class="page page_next"><a href="?page=' . $nextPage . $subString . '">다음</a></li>';
		}
		
		//마지막 페이지가 아니라면 끝 버튼을 생성
		if($page != $allPage) { 
			$paging .= '<li class="page page_end"><a href="?page=' . $allPage . $subString . '">끝</a></li>';
		}
		$paging .= '</ul>';
...
<head> 
	...
	<article class="boardArticle">
		<div id="boardList">
			<table>
				...
				<tbody>
						...
						<tr>
							...
							<td class="title">
								<a href="./view2.php?bno=<?php echo $row['b_no']?>"><?php echo $row['b_title']?></a>
							</td>
			...
			<div class="btnSet">
				<a href="./write2.php" class="btnWrite btn">글쓰기</a>
			</div>
			<div class="paging">
				<?php echo $paging ?>
			</div>
			<div class="searchBox">
				<!-- <form action="./csrf_vuln" method="get" style='margin-left:65px'>
					 <select name="searchColumn" style='margin:20px; padding:auto'>
					...
					<button class="general" type="submit">검색</button>
				</form>
			</div>
		</div>
	</article>
